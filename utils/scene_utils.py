...
import copy
@torch.no_grad()
def render_training_image(scene, gaussians, viewpoints, render_func, pipe, background, stage, iteration, time_now, mlp2):
    def render(gaussians, viewpoint, path, scaling):
        # scaling_copy = gaussians._scaling
        render_pkg = render_func(viewpoint, gaussians, pipe, background, stage=stage, deformation_feature=mlp2)
        ...
        # Semantic_feature_map is a 128 x 640 x 960 feature map tensor
        semantic_feature_map_np = semantic_feature_map.cpu().numpy()  # Convert to numpy if needed
        c, h, w = semantic_feature_map_np.shape
        # Reshape to have pixels as rows and features as columns
        semantic_feature_map_np = semantic_feature_map_np.reshape(c, -1).T  # Shape: (640*960, 128)
        # Apply PCA to reduce to 3 dimensions (for RGB visualization)
        pca = PCA(n_components=3)
        semantic_feature_map_rgb = pca.fit_transform(semantic_feature_map_np)  # Shape: (640*960, 3)
        # Normalize each channel to the range [0, 1]
        epsilon = 1e-8
        semantic_feature_map_rgb -= semantic_feature_map_rgb.min(axis=0)
        semantic_feature_map_rgb /= (semantic_feature_map_rgb.max(axis=0) + epsilon)
        # Reshape back to (640, 960, 3)
        semantic_feature_map_rgb = semantic_feature_map_rgb.reshape(h, w, 3)

        image_np = np.concatenate((gt_np, image_np, semantic_feature_map_rgb), axis=1)
