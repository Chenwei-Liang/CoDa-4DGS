...
class CameraInfo(NamedTuple):
    ...
    c2w: np.array = None
    semantic_feature: torch.Tensor=None
...
def constructCameras_waymo(frames_list, white_background, mapper = {},
                           load_intrinsic=False, load_c2w=False, start_time = 50, original_start_time = 0, semantic_feature_path=""):
    cam_infos = []
    for idx, frame in enumerate(frames_list):
        ...
        if feat_map_path is not None:
            ...
            _semantic_feature_path = os.path.join(semantic_feature_path, image_name) + '_fmap_CxHxW.pt' 
            semantic_feature_name = os.path.basename(_semantic_feature_path).split(".")[0]
            semantic_feature = torch.load(_semantic_feature_path)
...
def readWaymoInfo(path, white_background, eval, extension=".png", use_bg_gs=False, 
                  ...
                  stride = 10 , original_start_time = 0, semantic_feature_path=""
                  ):
    ...
    for idx, t in enumerate(train_timestamps):
        frame_dict = dict(  time = time_line[t+start_time-original_start_time],   
    ...
    train_cam_infos = constructCameras_waymo(train_frames_list, white_background, timestamp_mapper, 
                                             load_intrinsic=load_intrinsic, load_c2w=load_c2w,start_time=start_time,original_start_time=original_start_time, semantic_feature_path=semantic_feature_path)
    print("Reading Test Transforms")
